...
template<typename Type>
void Search_tree<Type>::Node::balance() {
    int unbalanced = left_tree->height() - right_tree->height();
...
bool Search_tree<Type>::Node::insert( Type const &obj ) {
	if ( obj < node_value ) {
		...
		if ( right_tree == nullptr ) {
			...
			next_node = right_tree; //todo check, add equal case
			return true;
		} else {
			if ( right_tree->insert( obj ) ) {
				update_height();
				return true;
			} else {
				return false;
...
template <typename Type>
bool Search_tree<Type>::Node::erase( Type const &obj, typename Search_tree<Type>::Node *&to_this ) {
	if ( obj < node_value ) {
		if ( left_tree == nullptr ) {
			return false;
		} else {
		...
		if ( is_leaf() ) {
            // todo add: next_node->previous_node = previous_node;
